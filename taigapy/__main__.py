...
def get(args):
    taiga_client = init_taiga(args.token)
    local_file = taiga_client.download_to_cache(
        id=args.dataset_id,
...
if __name__ == "__main__":
    ...
    create_parser.add_argument(
        "-n", "--name", required=True, help="Name of the dataset to create"
    )
    create_parser.add_argument(
        "-d", "--description", help="Description of the dataset to create"
    )
    create_parser.add_argument(
        "-f",
        "--files",
    ...
    update_parser.add_argument(
        "-p", "--dataset_permaname", help="Dataset permaname to update"
    )
    update_parser.add_argument(
        "-v", "--dataset_version", help="Dataset version to update"
    )
    update_parser.add_argument(
        "-d",
        "--dataset_description",
    ...
    update_parser.add_argument(
        "-k", "--force_keep", action="store_true", help="Keep all the files"
    )
    update_parser.add_argument(
        "-r", "--force_remove", action="store_true", help="Remove all the files"
    )
    ...
    get_parser.add_argument(
        "-p", "--dataset_permaname", help="Dataset permaname to retrieve"
    )
    get_parser.add_argument(
        "-v", "--dataset_version", help="Dataset version to retrieve"
    )
    ...
    get_parser.add_argument(
        "-e", "--encoding", help="Encoding to use to decypher the file"
    )
    get_parser.add_argument(
        "-t",
        "--format",
    ...
    parser.add_argument(
        "-f", "--format", action="store_true", help="Print the available formats"
    )
    parser.add_argument("-t", "--token", help="Path to the user token")
